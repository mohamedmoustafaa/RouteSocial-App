...
export default function register() {
  ...
  .refine((date) => {
    ...
    return !isNaN(userDate) && userDate < currentDate;
    ...
    path : ["rePassword"],    // عشان نعرف انهي input اللي فيها مشكلة    
  })
...
{apiError && <h1 className="text-center bg-red-500  text-white rounded-md my-2 p-2 font-bold">{apiError}</h1>}
  ...
  <div className= "relative z-0 w-full mb-5 group">
      ...
      {formState.errors.name&&formState.touchedFields.name ? <p className="text-red-500 font-semibold text-center my-2"> {formState.errors.name.message} </p> : ""}
  </div>
  <div className= "relative z-0 w-full mb-5 group">
  ...
  <div className= "relative z-0 w-full mb-5 group">
      <input type="password" {...register("rePassword")} id="rePassword" className= "block py-2.5 px-0 w-full text-sm text-balck-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-black dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" "  />
  ...
  <div className= "relative z-0 w-full mb-5 group">
      <input type="date" {...register("dateOfBirth")} id="dateOfBirth" className= "block py-2.5 px-0 w-full text-sm text-black-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-black dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" "  />
      <label htmlFor=   "dateOfBirth" className= "peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6"> Enter your date of Birth</label>
  ...
  <div className="flex items-center mb-4">
    ...
    {formState.errors.gender ? <p className="text-red-500 font-semibold text-center my-2"> {formState.errors.gender.message} </p> : ""}
